...
echo "Running Fast Tests for Threshold Protocols"
echo "=========================================="
...
run_test "pkg/ecdsa"
run_test "pkg/hash"
run_test "pkg/math/arith"
run_test "pkg/math/polynomial"
run_test "pkg/math/sample"
run_test "pkg/paillier" 30s
...
for zk in affg affp dec elog enc encelg fac log logstar mod mul mulstar nth prm sch; do
    run_test "pkg/zk/$zk" 30s
done

echo ""
echo "3. Internal packages"
echo "--------------------"
...
run_test "protocols/lss" 30s
run_test "protocols/cmp" 30s
